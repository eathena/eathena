...
int mmo_auth( struct mmo_account* account , int fd){
	...
	mysql_real_escape_string(&mysql_handle,t_uid,account->userid,strlen(account->userid));


	sql_query("SELECT m.`id`, m.`name`, c.`converge_pass_hash`, md5(c.`converge_pass_salt`), m.`mgroup`, e.`field_1` FROM ibf_members m "
	...
	if ((sql_row = mysql_fetch_row(sql_res))){
		...
		strcpy(ta.pass_hash,sql_row[2]);	//hash to compare against md5 (salt and password)
		strcpy(ta.pass_salt,sql_row[3]);	//salt to mix in with hashed password for hash
...
//	printf("uid: %d | uname: %s | pass_hash: %s | salt: %s | group: %d | sex: %d \n input username: %s || before escaping: %s\n", ta.id, ta.userid, ta.pass_hash, ta.pass_salt, ta.mgroup,ta.sex,t_uid,account->userid);
	...
	sql_query("SELECT MD5('%s%s')", ta.pass_salt, hashed_passwd);
	sql_res = mysql_store_result(&mysql_handle);
	if ((sql_row = mysql_fetch_row(sql_res))){
		strcpy(hashed_together,sql_row[0]);
		mysql_free_result(sql_res);
	}


	if (strcmp(hashed_together, ta.pass_hash))
		return 1;
...
void parse_login_charserver(int fd){
	...
	if(result == -1 && account.sex==2 && server_fd[account.account_id]==-1){
		...
		mysql_real_escape_string(&mysql_handle,t_uid,server[account.account_id].name,strlen(server[account.account_id].name));
