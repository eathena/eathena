...
int clif_set0078(struct map_session_data &sd, unsigned char *buf)
{
	...
	{	// mob disguises [Valaris]
		...
		WBUFB(buf,50) = 5;
		WBUFB(buf,51) = 0;
		WBUFW(buf,52) = ((level = status_get_lv(&sd.bl)) > battle_config.max_base_level) ? battle_config.max_base_level : level;
...
int clif_weather2(unsigned short m, int type)
{
	...
	for(i = 0; i < fd_max; i++)
	{
		if( session[i] && (sd = (struct map_session_data *)session[i]->session_data)!=NULL && session_isActive(sd->fd) && sd->state.auth && sd->bl.m == m)
		{
...
int clif_spawnpc(struct map_session_data &sd)
{
	...
	if(sd.state.viewsize==2) // tiny/big players [Valaris]
		...
		clif_specialeffect(sd.bl,EFFECT_TINY,0);
...
int clif_spawnmob(struct mob_data &md)
{
	...
	if(md.state.size==2) // tiny/big mobs [Valaris]
		clif_specialeffect(md.bl,EFFECT_BIG,0);
	else if(md.state.size==1)
		clif_specialeffect(md.bl,EFFECT_TINY,0);
...
int clif_changechatstatus(struct chat_data &cd)
{
	...
	WBUFB(buf,16)=cd.pub;
	strcpy((char*)WBUFP(buf,17),cd.title);
	clif_send(buf,strlen(cd.title)+17,&cd.usersd[0]->bl,CHAT);
...
int clif_parse_WantToConnection(int fd, struct map_session_data &sd)
{
	...
	unsigned char sex;
	size_t packet_ver=0;	// 5: old, 6: 7july04, 7: 13july04, 8: 26july04, 9: 9aug04/16aug04/17aug04, 10: 6sept04, 11: 21sept04, 12: 18oct04, 13: 25oct04 (by [Yor])

	if( !session_isActive(fd) )
		return 0;

	cmd = RFIFOW(fd,0);
	// ShowMessage("Received bytes %d with packet 0x%d.\n", RFIFOREST(fd), cmd);
	...
	{
		struct map_session_data *plsd = (struct map_session_data *) (session[fd]->session_data = aCalloc(1,sizeof(struct map_session_data)));
		plsd->fd = fd;
		plsd->packet_ver = packet_ver;
...
int clif_parse_TickSend(int fd, struct map_session_data &sd)
{
	...
	if( clif_config.enable_packet_db && sd.packet_ver == clif_config.packet_db_ver )
	{
...
int clif_friendslist_send(struct map_session_data &sd)
{
	...
	for(i=0,n=4; i<MAX_FRIENDLIST; i++)
	{
		if(sd.status.friends[i].char_id){
			WFIFOL(sd.fd, n+0) = (map_charid2sd(sd.status.friends[i].char_id) != NULL);
			WFIFOL(sd.fd, n+4) = sd.status.friends[i].char_id;
			memcpy(WFIFOP(sd.fd, n+8), &sd.status.friends[i].name, 24);
			n += 32;
		}
}
	WFIFOW(sd.fd,2) = n;
...
//	ShowMessage ("reply: %d %d %d\n", char_id, id, reply);
	...
	else
	{
		...
		else
		{
			...
			memset(f_sd->status.friends[i].name, 0, sizeof(f_sd->status.friends[i].name));
			memcpy(f_sd->status.friends[i].name, sd.status.name, 23);
...
{	// 0x203 </o> <ID to be removed W 4B>
	...
	for (i = 0; i < MAX_FRIENDLIST; i ++)
	{
		if(sd.status.friends[i].char_id == id)
		{
			// move all chars down
			for(j = i + 1; j < MAX_FRIENDLIST; j++){
				sd.status.friends[j-1].char_id = sd.status.friends[j].char_id;
				memcpy(sd.status.friends[j-1].name, sd.status.friends[j].name, sizeof(sd.status.friends[j].name));
			}
			// clear the last one
			sd.status.friends[MAX_FRIENDLIST-1].char_id = 0;
			memset(sd.status.friends[MAX_FRIENDLIST-1].name, 0, sizeof(sd.status.friends[MAX_FRIENDLIST-1].name));
	...
	while(session_isActive(fd) && RFIFOREST(fd) >= 2)
	{
		...
		{	// check authentification packet to know packet version
			...
			else if (cmd == 0xF5)
			{
				...
				else if (RFIFOREST(fd) >= 29 && (RFIFOB(fd,28) == 0 || RFIFOB(fd,28) == 1))
				{	// 00 = Female, 01 = Male
