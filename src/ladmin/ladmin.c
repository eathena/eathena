...
int typepasswd(char * password) {
	...
	if (defaultlanguage == 'F')
		ShowMessage(CL_BT_CYAN" Entrez le mot de passe > "CL_PASS"");
	else
		ShowMessage(CL_BT_CYAN" Type the password > "CL_PASS"");
		i = 0;
		while ((letter = getchar()) != '\n')
			password1[i++] = letter;
	if (defaultlanguage == 'F')
		ShowMessage(""CL_NORM""CL_BT_CYAN" Ré-entrez le mot de passe > "CL_PASS"");
	else
		ShowMessage(""CL_NORM""CL_BT_CYAN" Verify the password > "CL_PASS"");
		i = 0;
		while ((letter = getchar()) != '\n')
			password2[i++] = letter;
...
// general commands
			...
			ShowMessage("  NOTE: Server will never sends back a password.\n");
...
int delaccount(char* param) {
	...
	while ((confirm[0] != 'o' || defaultlanguage != 'F') && confirm[0] != 'n' && (confirm[0] != 'y' || defaultlanguage == 'F')) {
		if (defaultlanguage == 'F')
			ShowMessage(CL_BT_CYAN" ** Etes-vous vraiment sûr de vouloir SUPPRIMER le compte [$userid]? (o/n) > "CL_NORM"");
		else
			ShowMessage(CL_BT_CYAN" ** Are you really sure to DELETE account [$userid]? (y/n) > "CL_NORM"");
		fflush(stdout);
		memset(confirm, '\0', sizeof(confirm));
		i = 0;
		while ((letter = getchar()) != '\n')
...
int changememo(char* param) {
	...
	if (strlen(memo) > 254) {
		if (defaultlanguage == 'F') {
			ShowMessage("Mémo trop long (%d caractères).\n", strlen(memo));
			...
			ShowMessage("Memo is too long (%d characters).\n", strlen(memo));
			...
			if (RFIFOW(fd,2) < 5) {
				...
				for(i = 4; i < RFIFOW(fd,2); i += 38) {
					    ...
					    (list_type == 4 && RFIFOL(fd,i+34) == 0))
					{
						...
						ShowMessage("%6ld ", (unsigned long)RFIFOL(fd,i+30));
				...
				return 0;
		  {
			...
			if( (int)RFIFOL(fd,2) == -1 ) {
				...
				case 9:
					...
					break;
				default: // 100
					ShowMessage(" Statut: %ld [This ID is totally erased]\n", (unsigned long)RFIFOL(fd,36));
					break;
				}
				if (defaultlanguage == 'F') {
					if (ban_until_time == 0) {
					...
					if (RFIFOL(fd,32) > 1)
						ShowMessage(" Compteur: %ld connexions.\n", (unsigned long)RFIFOL(fd,32));
					else
						ShowMessage(" Compteur: %ld connexion.\n", (unsigned long)RFIFOL(fd,32));
					...
					if (RFIFOL(fd,32) > 1)
						ShowMessage(" Count:  %ld connections.\n", (unsigned long)RFIFOL(fd,32));
					else
						ShowMessage(" Count:  %ld connection.\n", (unsigned long)RFIFOL(fd,32));
...
int ladmin_config_read(const char *cfgName) {
	...
	if (defaultlanguage == 'F') {
		ShowMessage(CL_NORM"---Début de lecture du fichier de configuration Ladmin (%s)\n", cfgName);
	} else {
...
int do_init(int argc, char **argv) {
	...
	if (defaultlanguage == 'F') {
		...
		ShowMessage("Ladmin est "CL_BT_GREEN"prêt"CL_NORM".\n\n");
	} else {
		ladmin_log("Ladmin is ready." RETCODE);
		ShowMessage("Ladmin is "CL_BT_GREEN"ready"CL_NORM".\n\n");
	}
