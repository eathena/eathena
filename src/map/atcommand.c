...
#define prison_db	"lege_jail_records"
#define STATE_BLIND 0x10

static char command_symbol = '@'; // first char of the commands (by [Yor])
void GMchat(char*, char*);
void GMchat2(char*, char*);
...
ACMD_FUNC(gm_mes); // by Aru
ACMD_FUNC(ice); // by Aru
ACMD_FUNC(unice); // by Aru
ACMD_FUNC(god);	// by Aru
ACMD_FUNC(break);
ACMD_FUNC(pet16);
#ifdef MAIL
...
//	{ AtCommand_Disablenpc,			"@disablenpc",		80, atcommand_disablenpc }, // []
	...
	{ AtCommand_Disguise2,			"@disguise2",		99, atcommand_disguise2 }, // [Valaris]
...
//	{ AtCommand_ChangeSex,			"@changesex",		 1, atcommand_changesex }, // by MC Cameri
	...
	{ AtCommand_GMb,		"@gmb",		10,	atcommand_gm_mes },
	{ AtCommand_Ice,		"@ice",		10,	atcommand_ice },
	{ AtCommand_UnIce,	"@unice",		10,	atcommand_unice },
	{ AtCommand_God,	"@god",			40,	atcommand_god },
	{ AtCommand_Break,	"@break",		40,	atcommand_break },
	{ AtCommand_Pet16,	"@pet16",		99,	atcommand_pet16 },
	...
	{ AtCommand_FakeSC,			"@fakesc",		99, atcommand_fakesc },
...
is_atcommand(const int fd, struct map_session_data* sd, const char* message, int gmlvl) {
	...
	if (*str == '!' && pc_isGM(sd) >= get_atcommand_level(AtCommand_GMb))
	{
		str++;
		GMchat(sd->status.name,str);
		return AtCommand_Unknown;
	}


	type = atcommand(sd, gmlvl >= lowest_gm_level ? gmlvl : pc_isGM(sd), str, &info);
	...
	const char* command, const char* message)
{
	...
	for (i = 0; i < fd_max; i++) {
		if (session[i] && (pl_sd = (struct map_session_data *) session[i]->session_data) && pl_sd->state.auth) {
			pl_GM_level = pc_isGM(pl_sd);
			if (pl_GM_level > lowest_gm_level) {
				if (!((battle_config.hide_GM_session || (pl_sd->status.option & OPTION_HIDE)) && (pl_GM_level > GM_level))) { // you can look only lower or same level
					memcpy(player_name, pl_sd->status.name, 24);
					for (j = 0; player_name[j]; j++)
						player_name[j] = tolower(player_name[j]);
					if (strstr(player_name, match_text) != NULL) { // search with no case sensitive
						sprintf(atcmd_output, "Name: %s (GM:%d)", pl_sd->status.name, pl_GM_level);
						clif_displaymessage(fd, atcmd_output);
#ifdef EXTRA_WHOGM_INFO
	...
	if (!message || !*message || sscanf(message, "%99s", map_name) < 1 || town < -3 || town >= (int)(sizeof(data) / sizeof(data[0]))) {
		...
		if (town >= -3 && town <= -1) {
			if (sd->status.memo_point[-town-1].map[0]) {
				...
				if(pc_isGM(sd) < get_atcommand_level(AtCommand_Warp))
				{
					clif_displaymessage(fd, "Your GM level doesn't authoruze you to warp here.");
					return -1;
				}
			...
			if(pc_isGM(sd) < get_atcommand_level(AtCommand_Warp) && !map[sd->bl.m].flag.nobranch)
			{
				clif_displaymessage(fd,"Your GM level only allows warps from town.");
				return -1;
			}
	...
	const char* command, const char* message)
{
	...
	if (count != 0)
	{
		...
		sprintf(broadcastbuf,"summoned %d monsters on %s",count,map[sd->bl.m].name);
		GMchat2(sd->status.name,broadcastbuf);
	}
	else {
		clif_displaymessage(fd, msg_table[40]); // Invalid monster ID or name.
		return -1;
...
void add_jail_record(struct map_session_data* gm_sd,struct map_session_data* pl_sd,char* reason){
	char msg_buf[200];
	int i = 0;

// =====================================
// Tell prisoner who jailed them and why

	if(reason != NULL)
		sprintf(msg_buf, "GM %s has jailed you for %s",
			gm_sd->status.name, reason);
	else
		sprintf(msg_buf, "GM %s has jailed you for a rule violation",
			gm_sd->status.name);

	clif_displaymessage(pl_sd->fd, msg_buf);

	if(reason != NULL)
		sprintf (msg_buf, "jailed %s for %s",pl_sd->status.name, reason);
	else
		sprintf (msg_buf, "jailed %s for no reason",pl_sd->status.name);

	GMchat2(gm_sd->status.name, msg_buf);

// =====================================
// Display command results to GM

	if(pc_readglobalreg(pl_sd,"PC_JAILED")>0)	{
		sprintf(msg_buf, "Returned escaped prisoner %s",pl_sd->status.name);
		clif_displaymessage(gm_sd->fd, msg_buf);
		return;
	} else {
		pc_setglobalreg(pl_sd,"PC_JAILED",1);
		if(reason != NULL)
			sprintf(msg_buf,"Jailed %s for %s", pl_sd->status.name, reason);
		else
			sprintf(msg_buf,"Jailed %s", pl_sd->status.name);
		clif_displaymessage(gm_sd->fd, msg_buf);
	}

// =====================================
// Add entry to prison records db
	if(reason != NULL)
		sprintf(tmp_sql,"INSERT into `%s` (`account_id`, `char_id`, `name`, `gm_account_id`, `gm_name`, `time`, `reason`)"
			"VALUES ('%d', '%d', '%s', '%d', '%s', UNIX_TIMESTAMP(), '%s')",
			prison_db, pl_sd->status.account_id, pl_sd->status.char_id,
			pl_sd->status.name, gm_sd->status.account_id, gm_sd->status.name,
			reason);
	else
		sprintf(tmp_sql,"INSERT into `%s` (`account_id`, `char_id`,`name`, `gm_account_id`, `gm_name`, `time`)"
			"VALUES ('%d', '%d', '%s', '%d', '%s', UNIX_TIMESTAMP())",
			prison_db, pl_sd->status.account_id, pl_sd->status.char_id,
			pl_sd->status.name, gm_sd->status.account_id, gm_sd->status.name);

	while(mysql_query(&mmysql_handle, tmp_sql) && i < 5)
	{
		i++;
		printf("Error: Insert jail_record, will retry %d times: %s\n",
			i,mysql_error(&mmysql_handle));
	}
	...
	const char* command, const char* message)
{
	...
	struct map_session_data *pl_sd;
	int x, y;
	int mes_i, res_i, com_i, in_reason;

	memset(character, '\0', sizeof(character));
	memset(com, '\0', sizeof(character));
	memset(reason, '\0', sizeof(character));

	mes_i = res_i = com_i = in_reason = 0;

	while(message[mes_i] != '\0')
	{
		if(message[mes_i] == '\'') {
			if(in_reason) {
				in_reason = 0;
		...
		if(in_reason) {
			reason[res_i] = message[mes_i];
			res_i++;
		} else {
			com[com_i] = message[mes_i];
			com_i++;
		}
		mes_i++;
	}
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Please, enter a player name (usage: @jail ['reason'] <char_name>).");
		return -1;
	}
	...
	if ((pl_sd = map_nick2sd(character)) != NULL) {
		if (pc_isGM(sd) >= pc_isGM(pl_sd)) { // you can jail only lower or same GM
			...
			if (pc_setpos(pl_sd, "sec_pri.gat", x, y, 3) == 0) {
				...
				if(reason[0]!='\0')
					add_jail_record(sd, pl_sd, reason);
				else
					add_jail_record(sd, pl_sd, NULL);
			} else {
				clif_displaymessage(fd, msg_table[1]); // Map not found.
				return -1;
			}
	...
	if ((pl_sd = map_nick2sd(character)) != NULL) {
		if (pc_isGM(sd) >= pc_isGM(pl_sd)) { // you can jail only lower or same GM
			if (pl_sd->bl.m != map_mapname2mapid("sec_pri.gat")) {
				...
				pc_setglobalreg(pl_sd,"PC_JAILED",0);
				sprintf(tmp_sql,"DELETE from `%s` where `char_id` = '%d'",prison_db, pl_sd->status.char_id);
				while(mysql_query(&mmysql_handle, tmp_sql) && i < 5)
				{
					i++;
					printf("Error: Delete jail_record, will retry %d times: %s\n",
						i,mysql_error(&mmysql_handle));
				}
	...
	const char* command, const char* message)
{
	...
	if ((mob_id = mobdb_searchname(message)) == 0) // check name first (to avoid possible name begining by a number)
		mob_id = atoi(message);
#if 0
(mob_id >=  46 && mob_id <= 125) || (mob_id >= 700 && mob_id <= 718) || // NPC
	    (mob_id >= 721 && mob_id <= 755) || (mob_id >= 757 && mob_id <= 811) || // NPC
	    (mob_id >= 813 && mob_id <= 834) || // NPC
#endif

	if (mob_id > 1000 && mob_id <= 1690) { // monsters
...
/*==========================================
 * @disguise without sanity checks, by aru (FOR ADMINS ONLY!)
 *------------------------------------------
 */
int atcommand_disguise2(
	const int fd, struct map_session_data* sd,
	const char* command, const char* message)
{
	int mob_id = 0;
	...
	const char* command, const char* message)
{
	...
	if ((mob_id >=  46 && mob_id <= 125) || (mob_id >= 700 && mob_id <= 718) || // NPC
	    (mob_id >= 721 && mob_id <= 755) || (mob_id >= 757 && mob_id <= 811) || // NPC
	    (mob_id >= 813 && mob_id <= 834) || // NPC
	    (mob_id > 1000 && mob_id < 1625)) { // monsters
		for(i=0; i < fd_max; i++) {
			if(session[i] && (pl_sd = (struct map_session_data *) session[i]->session_data) && pl_sd->state.auth) {
...
#ifdef MAIL  /* Begin SQL-Only commands */
...
struct item_data {
	...
	int elv;
	int wlv;
	char *use_script;	// %Ò_'∆'c'‡'S"'+'Ã'+'≈'‚'Î'§'c'»'∆
	char *equip_script;	// _U_',-h_‰'Ã'R_<_›'Ë'‡'+'Ã'+'≈%¬"\'c'»?
	struct {
...
int atcommand_gm_mes(const int fd, struct map_session_data* sd,
	const char* command, const char* message)
{
	char text[100];
	...
	if (sscanf(message, "%99[^\n]", text) < 1)
	{
		clif_displaymessage(fd, "Please enter a message.");
		return -1;
	}

	GMchat(sd->status.name,text);
	...
	const char* command, const char* message)
{
	...
	status_calc_pc(pl_sd, 0);
	clif_displaymessage(fd, "Player frozen."); // Character's options changed.
        return 0;
    }
    return -1;
}
	...
	const char* command, const char* message)
{
    ...
    if((pl_sd=map_nick2sd((char *) message)) != NULL) {
	  ...
	  clif_changeoption(&pl_sd->bl);
  	  status_calc_pc(pl_sd, 0);
	clif_displaymessage(fd, "Player released."); // Character's options changed.
        return 0;
    }
    return -1;
}

void GMchat(char* name, char* text)
{
	char mes [150];
	struct map_session_data *pl_sd;
	int i;

	sprintf(mes, "<%s> %s",name,text);

	for (i = 0; i < fd_max; i++) {
		if (session[i] && (pl_sd = session[i]->session_data) && pl_sd->state.auth) {
			if (pc_isGM(pl_sd) >= get_atcommand_level(AtCommand_GMb)) {
				clif_GMmessage(&pl_sd->bl,mes,strlen(mes)+1,16|3);
			}
		}
	}
#if 0
	intif_GMmessage(mes, strlen(mes) + 1, 0);
#endif
}

void GMchat2(char* name, char* text)
{
	char mes [150];
	struct map_session_data *pl_sd;
	int i;
...
int atcommand_god(const int fd, struct map_session_data* sd,
	const char* command, const char* message)
{
	sd->state.god = !sd->state.god;

	if(sd->state.god)
		clif_displaymessage(fd, "Degreelessness Mode On.");
	else
		clif_displaymessage(fd, "Degreelessness Mode Off.");

	return 0;
}

int atcommand_break(const int fd, struct map_session_data* sd,
	const char* command, const char* message)
{
	int piece;

	if (!message || !*message || (piece = atoi(message)) < 1) {
		clif_displaymessage(fd, "Usage: @break n");
		clif_displaymessage(fd, "1: weapon  2: armor");
		clif_displaymessage(fd, "3: shield  4: helm");
		return -1;
	}
...
int atcommand_pet16(const int fd, struct map_session_data* sd,
	const char* command, const char* message)
{
	int piece;

	if (!message || !*message || (piece = atoi(message)) < 1) {
		clif_displaymessage(fd, "Usage: @pet16 n");
		return -1;
	}

	clif_displaymessage(fd,"Pet itendifier changed.");

	pet_16 = piece;
	return 0;
}

int atcommand_fakesc(const int fd, struct map_session_data* sd,
	const char* command, const char* message)
{
	int piece;

	if (!message || !*message || (piece = atoi(message)) < 0) {
		clif_displaymessage(fd, "Usage: @fakesc n");
		return -1;
	}

	clif_status_change(&sd->bl,piece,1);
	return 0;
}
