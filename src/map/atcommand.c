	...
	struct map_session_data* target_sd)
{
	...
	sprintf(output, " -- Player %s invites %s to duel -- ",
...
/*==========================================
 * @help2. Contains char commands [Kayla]
 *------------------------------------------
 */
int atcommand_help2(
	const int fd, struct map_session_data* sd,
	const char* command, const char* message)
{
	...
	if ((fp = fopen(help2_txt, "r")) != NULL) {
		...
		clif_displaymessage(fd, msg_table[27]); /*  File help2.txt not found. */
		return -1;
	}
	...
	const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.pvp) {
		...
		pl_allsd = map_getallusers(&users);
		for (i = 0; i < users; i++) {	//êlêîï™ÉãÅ[Év
	...
	const char* command, const char* message)
{
    ...
    */
	...
	if(!duel_checktime(sd)) {
		sprintf(output, "Duel: You can take part in duel only one time per %d minutes.",
