...
static int itemdb_read_sqldb(void)
{
	...
	for (i = 0; i < 2; i++) {
		printf("load item_db from sql\n");
		sprintf(tmp_sql, "SELECT `ID`, `Name`, `Jname`, `Type`, `Price`, `Sell`, `Weight`, `ATK`, `DEF`, `Range`, `Slot`, `Job`,`Gender`, `Loc`, `wLV`, `eLV`, `View`, `UseScript`, `EquipScript` FROM `%s`", item_db_name[i]);
