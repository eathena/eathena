...
int log_mvpdrop(struct map_session_data &sd, uint32 monster_id, int log_mvp[])
{
	...
	if(log_config.sql_logs > 0)
	{
		sprintf(tmp_sql, "INSERT DELAYED INTO `%s` (`mvp_date`, `kill_char_id`, `monster_id`, `prize`, `mvpexp`, `map`) VALUES (NOW(), '%ld', '%ld', '%d', '%d', '%s') ", log_config.log_mvpdrop_db, (unsigned long)sd.status.char_id, (unsigned long)monster_id, log_mvp[0], log_mvp[1], sd.mapname);
		if(mysql_SendQuery(&logmysql_handle, tmp_sql))
			ShowError("DB server Error - %s\n",mysql_error(&logmysql_handle));
	} else {
#endif
