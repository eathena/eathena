...
int pet_calc_pos(struct pet_data &pd,int tx,int ty,int dir)
{
	...
	pd.walkpath.target.x = x;
	pd.walkpath.target.y = y;
	return 0;
}
...
int pet_attack(struct pet_data &pd,unsigned int tick,int data)
{
	...
	pd.attackabletime = tick + status_get_adelay(&pd);
	...
	pd.timer=add_timer(pd.attackabletime,pet_timer,pd.block_list::id,0);
...
int petskill_use(struct pet_data &pd, struct block_list &target, short skill_id, short skill_lv, unsigned int tick)
{
	...
	if (casttime > 0)
	{
		...
		pd.timer = add_timer(pd.attackabletime,pet_timer,pd.block_list::id, basics::numptr(dat), false);
	}
	else
	{
...
int pet_walk(struct pet_data &pd,unsigned long tick,int data)
{
	...
	if(pd.walkpath.path_half==0){
		...
		if(pd.walkpath.change_walk_target){
			pet_walktoxy_sub(pd);
			return 0;
		}
	}
		...
		if(map_getcell(pd.block_list::m,x+dx,y+dy,CELL_CHKNOPASS)){
			pet_walktoxy_sub(pd);
			return 0;
		}
		...
		CMap::foreachinmovearea( CClifPetOutsight(pd),
			pd.block_list::m,x-AREA_SIZE,y-AREA_SIZE,x+AREA_SIZE,y+AREA_SIZE,dx,dy,BL_PC);
...
//			&pd);
		...
		if(moveblock) pd.map_delblock();
		pd.block_list::x = x;
		pd.block_list::y = y;
		if(moveblock) pd.map_addblock();
...
//			&pd);
	...
	if((i=calc_next_walk_step(pd))>0){
		...
		pd.timer=add_timer(tick+i,pet_timer,pd.block_list::id,pd.walkpath.path_pos);
	}
	return 0;
}
...
int pet_changestate(struct pet_data &pd,int state,int type)
{
	...
	switch(state)
	{
		case MS_WALK:
			if((i=calc_next_walk_step(pd)) > 0){
				i = i>>2;
				pd.timer=add_timer(gettick()+i,pet_timer,pd.block_list::id,0);
			} else
				pd.state.state=MS_IDLE;
			break;
		case MS_ATTACK:
			...
			if(i>0 && i<2000)
				...
				pd.timer=add_timer(tick+1,pet_timer,pd.block_list::id,0);
			break;
		case MS_DELAY:
				pd.timer=add_timer(gettick()+type,pet_timer,pd.block_list::id,0);
			break;
	}
	return 0;
}
...
int pet_timer(int tid, unsigned long tick, int id, basics::numptr data)
{
	...
	if(pd == NULL || pd->block_list::type != BL_PET)
		return 1;
...
int pet_walktoxy_sub(struct pet_data &pd)
{
	...
	if( !wpd.path_search(pd.block_list::m,pd.block_list::x,pd.block_list::y,pd.walkpath.target.x,pd.walkpath.target.y,0) )
		return 1;
	pd.walkpath = wpd;

	pd.walkpath.change_walk_target=0;
...
{
	if(pd.state.state == MS_WALK && !walkpath_data::is_possible(pd.block_list::m,pd.block_list::x,pd.block_list::y,x,y,0))
		return 1;
	...
	if(pd.state.state == MS_WALK) {
		pd.walkpath.change_walk_target=1;
	} else {
...
int pet_stop_walking(struct pet_data &pd,int type)
{
	if(pd.state.state == MS_WALK || pd.state.state == MS_IDLE) {
		...
		pd.walkpath.target.x=pd.block_list::x;
		pd.walkpath.target.y=pd.block_list::y;
	}
	if(type&0x01)
		clif_fixpetpos(pd);
	if(type&~0xff)
...
int pet_data_init(struct map_session_data &sd)
{
	...
	sd.petDB = &pet_db[i];
	sd.pd = pd = new struct pet_data(sd.pet.name);

	pd->block_list::m = sd.block_list::m;
	pd->block_list::x = pd->walkpath.target.x = sd.block_list::x;
	pd->block_list::y = pd->walkpath.target.y = sd.block_list::y;
	pet_calc_pos(*pd,sd.block_list::x,sd.block_list::y,sd.dir);
	pd->block_list::x = pd->walkpath.target.x;
	pd->block_list::y = pd->walkpath.target.y;
	...
	{
		pd->status = new struct pet_data::pet_status;
	}
...
int pet_randomwalk(struct pet_data &pd,unsigned long tick)
{
	...
	if(DIFF_TICK(pd.next_walktime,tick) < 0){
		...
		for(i=0;i<retrycount;++i)
		{
			...
			if((map_getcell(pd.block_list::m,x,y,CELL_CHKPASS))&&( pet_walktoxy(pd,x,y)==0)){
...
//			&pd,&i);
	...
	if(sd->pet.intimate > 0)
	{
		...
		if(dist > 12)
		{
			if(pd.target_id > 0)
				pet_unlocktarget(pd);
			if(pd.timer != -1 && pd.state.state == MS_WALK && sd->get_distance(pd.walkpath.target.x,pd.walkpath.target.y) < 3)
				return 0;
			pd.speed = sd->speed *3/4; // be faster than master
			if(pd.speed <= 10)
				pd.speed = 10;
			pet_calc_pos(pd,sd->block_list::x,sd->block_list::y,sd->dir);
			if(pet_walktoxy(pd,pd.walkpath.target.x,pd.walkpath.target.y))
...
//				pet_unlocktarget(pd);
			...
			{	//Skotlex Don't interrupt a casting spell when targed moved
				if(pd.timer != -1 && pd.state.state == MS_WALK && md->get_distance(pd.walkpath.target.x,pd.walkpath.target.y) < 2)
					return 0;
				...
				else
				{
					...
					do
					{
						...
						ret=pet_walktoxy(pd,pd.block_list::x+dx,pd.block_list::y+dy);
						i++;
					} while(ret && i<5);
					...
					{	// 移動不可能な所からの攻撃なら2歩下る
						...
						pet_walktoxy(pd,pd.block_list::x+dx,pd.block_list::y+dy);
					}
				}
			}
			else
		...
		{	//Item Targeted, attempt loot
			...
			{
				if(pd.timer != -1 && pd.state.state!=MS_ATTACK && (DIFF_TICK(pd.next_walktime,tick)<0 || bl_item->get_distance(pd.walkpath.target.x,pd.walkpath.target.y) <= 0))
					return 0; // 既に移動中
