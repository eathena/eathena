...
int npc_event_doall_sub(void *key,void *data,va_list ap)
{
	...
	if( (p=strchr(p,':')) && p && strcasecmp(name,p)==0 && (map<0 || map==ev->nd->bl.m))
	{
		if(ev->nd->u.scr.ref)
...
int npc_timerevent(int tid,unsigned long tick,int id,int data)
{
	...
	if(nd->u.scr.ref)
	CScriptEngine::run(nd->u.scr.ref->script,te->pos,nd->u.scr.rid,nd->bl.id);
	return 0;
}
/*==========================================
 * タイマーイベント開始
...
int npc_click(struct map_session_data &sd,unsigned long npcid)
{
	...
	case SCRIPT:
		if(nd->u.scr.ref)
			CScriptEngine::run(nd->u.scr.ref->script,0,sd.bl.id,npcid);
		break;
	}
