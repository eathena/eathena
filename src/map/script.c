...
int buildin_options(struct script_state *st); // Special options [Aru]
...
//	{buildin_petmag,"petmag","iiii"}, // [Valaris]
	...
	{buildin_options,"reloadoptions",""}, // advanced options	// Aru
...
int buildin_getcharid(struct script_state *st)
{
	...
	if(num==10)
		push_val(st->stack,C_INT,GetWord(sd->status.char_id,0));
	if(num==11)
		push_val(st->stack,C_INT,GetWord(sd->status.char_id,1));
	return 0;
}
/*==========================================
 *指定IDのPT名取得
...
int buildin_pvpon(struct script_state *st)
{
	...
	if(m >= 0 && !map[m].flag.pvp && !map[m].flag.nopvp) {
		...
		if(map[m].flag.pkmode) // disable ranking functions if pk_mode is on [Valaris]
			return 0;
...
int buildin_pvpoff(struct script_state *st)
{
	...
	if(m >= 0 && map[m].flag.pvp && !map[m].flag.nopvp) { //fixed Lupus
		...
		if(map[m].flag.pkmode) // disable ranking options if pk_mode is on [Valaris]
			return 0;
...
int buildin_petskillbonus(struct script_state *st)
{
	...
	if (battle_config.pet_equip_required && (pd->equip != sd->petDB->AcceID))
		pd->bonus->timer=-1;
	else
...
int buildin_options(struct script_state *st)
{
	struct map_session_data *sd=script_rid2sd(st);
...
//	sd->autoloot = pc_readglobalreg(sd,"autoloot");	
	sd->autoloot = 0;
	sd->show_bexp = pc_readglobalreg(sd,"showbase");
	sd->show_jexp = pc_readglobalreg(sd,"showjob");
