...
int status_calc_pc(struct map_session_data* sd,int first)
{
	...
	if(sd->equip_index[10] >= 0){ // ñÓ
		...
		if(sd->inventory_data[index] && sd->status.inventory[index].equip&0x8000){		// Arrows
			sd->state.lr_flag = 2;
			run_script(sd->inventory_data[index]->script,0,sd->bl.id,0);
			sd->state.lr_flag = 0;
			if (!calculating) //Abort, run_script retriggered status_calc_pc. [Skotlex]
...
int status_get_sc_tick(struct block_list *bl, int type, int tick)
{
	...
	switch (type) {
		...
		break;
		case SC_BLIND:
			rate = 50*status_get_vit(bl) +50*status_get_int(bl);
		break;
		case SC_CONFUSION:
			rate = 50*status_get_str(bl) +50*status_get_int(bl);
		break;
		case SC_SWOO:
...
// security system to prevent forgetting timer removal
	...
	case SC_MARIONETTE2:
		{
			struct block_list *pbl = map_id2bl(sc->data[type].val3);
			if (pbl && check_distance_bl(bl, pbl, 7) && (sc->data[type].val2 -= 1000)>0) {
				sc->data[type].timer = add_timer(
					1000 + tick, status_change_timer,
					bl->id, data);
					return 0;
