	...
	case AREA_WOSC:
		{
			...
			if(sd.chatID && sd.chatID == ssd.chatID)
				return 0;
		}
		break;
	}
...
int clif_clearchar(struct map_session_data &sd, struct block_list &bl)
{
	...
	WBUFL(buf,2) = bl.id;
	WBUFB(buf,6) = 0;
	clif_send(buf, packet_len_table[0x80], &sd, SELF);
...
int clif_mob007b(struct mob_data &md, unsigned char *buf)
{
	...
	{	// Use 0x1da packet for monsters with player sprites [Valaris]
		...
		WBUFPOS2(buf,50,md.block_list::x,md.block_list::y,md.target.x,md.target.y);
		WBUFB(buf,55)=0x88; // Deals with acceleration in directions. [Valaris]
		WBUFB(buf,56)=5;
		WBUFB(buf,57)=5;
...
int clif_npc007b(struct npc_data &nd, unsigned char *buf)
{
	...
	WBUFL(buf,22)=gettick();
	WBUFPOS2(buf,50,nd.block_list::x,nd.block_list::y,nd.target.x,nd.target.y);
	WBUFB(buf,55)=0x88; // Deals with acceleration in directions. [Valaris]
	WBUFB(buf,56)=5;
	WBUFB(buf,57)=5;
...
int clif_pet007b(struct pet_data &pd, unsigned char *buf)
{
	...
	{	// Use 0x1da packet for monsters with player sprites [Valaris]
		...
		WBUFPOS2(buf,50,pd.block_list::x,pd.block_list::y,pd.target.x,pd.target.y);
		WBUFB(buf,55)=0x88; // Deals with acceleration in directions. [Valaris]
		WBUFB(buf,56)=0;
		WBUFB(buf,57)=0;
		WBUFW(buf,58)=(level>battle_config.max_base_level)? battle_config.max_base_level:level;
...
int clif_spawnnpc(struct map_session_data &sd, struct npc_data &nd)
{
	...
	clif_send(buf,packet_len_table[0x7c],&sd,SELF);

	len = clif_npc0078(nd,buf);
	clif_send(buf,len,&sd,SELF);
...
int clif_moveobject(const block_list& bl)
{
	if( bl.get_sd() )
	{
			...
			maps[sd.block_list::m].flag.rain )
		{
			...
			WBUFPOS2(buf2,50,sd.block_list::x,sd.block_list::y,sd.target.x,sd.target.y);
			WBUFB(buf2,56)=5;
			WBUFB(buf2,57)=5;
...
int clif_fixobject(const block_list &bl)
{
	...
	if( bl.block_list::get_sd() )
	{
		map_session_data &sd = *bl.block_list::get_sd();
		if( sd.is_walking() )
			len = clif_set007b(sd,buf);
		else
			len = clif_set0078(sd,buf);
	}
	if( bl.block_list::get_md() )
	{
...
int clif_pcoutsight(struct block_list &bl,va_list &ap)
{
	...
	case BL_PC:
	{
		...
		if(sd->block_list::id != dstsd.block_list::id)
		{
			clif_clearchar_id(sd->fd, dstsd.block_list::id, 0);
			clif_clearchar_id(dstsd.fd, sd->block_list::id, 0);

			if(dstsd.disguise_id || sd->disguise_id)
			{
				clif_clearchar_id(sd->fd, dstsd.block_list::id|FLAG_DISGUISE, 0);
				clif_clearchar_id(dstsd.fd, sd->block_list::id|FLAG_DISGUISE, 0);
			}
...
int clif_moboutsight(struct block_list &bl,va_list &ap)
{
	...
	if(bl.type==BL_PC && session[sd.fd] != NULL)
	{
		clif_clearchar_id(sd.fd,md->block_list::id,0);
	}
...
int clif_petoutsight(struct block_list &bl,va_list &ap)
{
	...
	if(bl.type==BL_PC && session[sd.fd] != NULL) {
		clif_clearchar_id(sd.fd,pd->block_list::id,0);
	}
	return 0;
}
*/
...
int clif_npcoutsight(struct block_list &bl,va_list &ap)
{
	...
	if(bl.type==BL_PC && session[sd.fd] != NULL) {
		clif_clearchar_id(sd.fd,nd->block_list::id,0);
	}
...
int CClifOutsight::process(struct block_list& bl) const
{
	...
	{	//sd has lost sight of object
		...
		case BL_PC:
		{
			...
			clif_clearchar_id(osd->fd,  sd->block_list::id, 0);
			clif_clearchar_id( sd->fd, osd->block_list::id, 0);

			if( osd->disguise_id )
				clif_clearchar_id( sd->fd, osd->block_list::id|FLAG_DISGUISE, 0);
			if( sd->disguise_id )
				clif_clearchar_id(osd->fd,  sd->block_list::id|FLAG_DISGUISE, 0);
...
int clif_pvpset(struct map_session_data &sd,uint32 pvprank,uint32 pvpnum,int type)
{
	...
	if(type == 2) {
		...
		if(!type)
			...
			clif_send(buf,packet_len_table[0x19a],&sd,ALL_SAMEMAP);
	}
...
int clif_changed_dir(struct block_list &bl)
{
	...
	WBUFW(buf,0) = 0x9c;
	WBUFL(buf,2) = bl.id;
	WBUFW(buf,6) = status_get_headdir(&bl);
...
int clif_parse_LGMmessage(int fd, struct map_session_data &sd) {
	    ...
	    sd.isGM() >= get_atcommand_level(AtCommand_LocalBroadcast) )
	{
		...
		clif_send(buf, RFIFOW(fd,2), &sd, ALL_SAMEMAP);
	}
	return 0;
}
...
int clif_parse_HomMenu(int fd, struct map_session_data &sd)
{
	if( !session_isActive(fd) )
		return 0;
//	unsigned short cmd	= RFIFOW(fd,0);
//	homun_menu(sd,RFIFOB(fd,packet_db[sd.packet_ver][cmd].pos[0]));
	return 0;
}
...
int clif_parse_HomWalkToXY(int fd, struct map_session_data &sd)
{
	...
	unsigned short cmd = RFIFOW(fd,0);
//	int id = RFIFOL(fd,2);
	uchar v0 = RFIFOB(fd,0+packet_db[sd.packet_ver][cmd].pos[0]);
	uchar v1 = RFIFOB(fd,1+packet_db[sd.packet_ver][cmd].pos[0]);
	uchar v2 = RFIFOB(fd,2+packet_db[sd.packet_ver][cmd].pos[0]);

	x = ( v0      <<2) | (v1>>6);	// inverse to position encoding
	y = ((v1&0x3f)<<4) | (v2>>4);

	//walktoxy(&sd->hd->bl,x,y);
	return 0;
}
...
/*
	...
	target_id = RFIFOL(fd,packet_db[sd.packet_ver][cmd].pos[0]);
	action_type = RFIFOB(fd,packet_db[sd.packet_ver][cmd].pos[1]);
	...
	{
	case 0x00:	// once attack
	case 0x07:	// continuous attack ...???e?I?c?E?H
		{
...
//	homun_change_name(sd,RFIFOP(fd,packet_db[sd.packet_ver][cmd].pos[0]));
	return 0;
}
...
int clif_parse(int fd)
{
	...
	while(session_isActive(fd) && RFIFOREST(fd) >= 2)
	{
		...
		{	// 不明なパケット
			...
			time_t now;
			fp = basics::safefopen(packet_txt, "a");
			if(fp == NULL)
			{
...
//		{clif_parse_RankingPk,"rankingpk"},
	...
	while( fgets(line,sizeof(line),fp) )
	{
		...
		{	// only read valid packet_ver's
			...
			if(str[3]==NULL){
				ShowError("packet_db: packet error (line %i\n)", ln);
				exit(1);
			}
