...
int status_calc_pc(map_session_data& sd, int first)
{
	...
	if (calculating < 10) //Too many recursive calls to status_calc_pc!
	{
...

		{
			...
			if( sd.has_status(SC_GLORIA) )	// ƒOƒƒŠƒA
				sd.paramb[5]+= 30;
			if( sd.has_status(SC_LOUD) )	// ƒ‰ƒEƒhƒ{ƒCƒX
				sd.paramb[0]+= 4;
		...
		if( sd.has_status(SC_SPEARSQUICKEN) && !sd.has_status(SC_ADRENALINE) &&
			...
			!sd.has_status(SC_DONTFORGETME) )	// ƒXƒsƒAƒNƒBƒbƒPƒ“
			aspd_rate -= sd.sc_data[SC_SPEARSQUICKEN].integer2();
		if( sd.has_status(SC_ASSNCROS) && // —[—z‚ÌƒAƒTƒVƒ“ƒNƒƒX
		...
		if(	sd.has_status( (status_t)(i=SC_SPEEDPOTION3) ) ||
			...
			sd.has_status( (status_t)(i=SC_SPEEDPOTION0) ) )	// ? ‘¬ƒ|?ƒVƒ‡ƒ“
			aspd_rate -= sd.sc_data[i].integer2();
		if( sd.has_status(SC_GRAVITATION) )
			aspd_rate += sd.sc_data[SC_GRAVITATION].integer2();
		if( sd.has_status(SC_WINDWALK) && !sd.has_status(SC_INCREASEAGI) )	//ƒEƒBƒ“ƒhƒEƒH?ƒNbÍLv*2%Œ¸Z
			sd.speed_rate -= sd.sc_data[SC_WINDWALK].integer1()*4;
		if( sd.has_status(SC_CARTBOOST) )	// ƒJ?ƒgƒu?ƒXƒg
			sd.speed_rate -= 20;
		if( sd.has_status(SC_BERSERK) )	//ƒo?ƒT?ƒN’†‚ÍIA‚Æ“¯‚¶‚®‚ç‚¢‘¬‚¢H
			sd.speed_rate -= 25;
		if( sd.has_status(SC_WEDDING) )	//Œ‹¥’†‚Í?‚­‚Ì‚ª?‚¢
			sd.speed_rate += 100;
		...
		if( sd.has_status(SC_SPIDERWEB) ) //ƒXƒpƒCƒ_?ƒEƒFƒu
		{
			sd.flee = sd.flee * 50 / 100;
		}
		if( sd.has_status(SC_TRUESIGHT) ) //ƒgƒDƒ‹?ƒTƒCƒg
			sd.hit += 3 * sd.sc_data[SC_TRUESIGHT].integer1();
		if( sd.has_status(SC_CONCENTRATION) ) //ƒRƒ“ƒZƒ“ƒgƒŒ?ƒVƒ‡ƒ“
		...
		{	// •s€g‚ÌƒW?ƒNƒtƒŠ?ƒh
			sd.subele[1] += sd.sc_data[SC_SIEGFRIED].integer2();	// …
			sd.subele[2] += sd.sc_data[SC_SIEGFRIED].integer2();	// …
			sd.subele[3] += sd.sc_data[SC_SIEGFRIED].integer2();	// ‰Î
			sd.subele[4] += sd.sc_data[SC_SIEGFRIED].integer2();	// …
			sd.subele[5] += sd.sc_data[SC_SIEGFRIED].integer2();	// …
			sd.subele[6] += sd.sc_data[SC_SIEGFRIED].integer2();	// …
			sd.subele[7] += sd.sc_data[SC_SIEGFRIED].integer2();	// …
			sd.subele[8] += sd.sc_data[SC_SIEGFRIED].integer2();	// …
			sd.subele[9] += sd.sc_data[SC_SIEGFRIED].integer2();	// …
		}
		if( sd.has_status(SC_PROVIDENCE) )
		{	// ƒvƒƒ”ƒBƒfƒ“ƒX
			sd.subele[6] += sd.sc_data[SC_PROVIDENCE].integer2();	// ? ¹?«
			sd.subrace[6] += sd.sc_data[SC_PROVIDENCE].integer2();	// ? ?–‚
		}
		...
		if( sd.has_status(SC_DELUGE) && sd.def_ele==1){	// ƒfƒŠƒ…?ƒW
		...
		if( sd.has_status(SC_FORTUNE) )	// K‰^‚ÌƒLƒX
		...
		if(sc_data) {
			...
			if( sc_data[SC_BLESSING].timer != -1){	// ƒuƒŒƒbƒVƒ“ƒO
				...
				if( bl->is_undead() || race == 6)
					str >>= 1;	// ˆ« –‚/•s€
				else str += sc_data[SC_BLESSING].integer1();	// ‚»‚Ì‘¼
			}
		...
		if(sc_data) {
			if(sc_data[SC_BLESSING].timer != -1){	// ƒuƒŒƒbƒVƒ“ƒO
				...
				if(bl->is_undead() || race == 6 )
					...
					int_ += sc_data[SC_BLESSING].integer1();	// ‚»‚Ì‘¼
			}
	...
	else
	{
		...
		if(sc_data)
		{
			...
			if(sc_data[SC_BLESSING].timer != -1){	// ƒuƒŒƒbƒVƒ“ƒO
				...
				if( bl->is_undead() || race == 6 )
					dex >>= 1;	// ˆ« –‚/•s€
				else dex += sc_data[SC_BLESSING].integer1();	// ‚»‚Ì‘¼
			}
			if(sc_data[SC_QUAGMIRE].timer!=-1)	{ // ƒNƒ@ƒOƒ}ƒCƒA
				// dex >>= 1;
	...
	{	//‚»‚êˆÈŠO‚È‚ç
		...
		{	//ó‘ÔˆÙí‚ ‚è
			if(sc_data[SC_PROVOKE].timer!=-1) //PC‚Åƒvƒƒ{ƒbƒN(SM_PROVOKE)ó‘Ô
				batk += batk *(2+3*sc_data[SC_PROVOKE].integer1())/100; //base_atk‘‰Á
...
int status_change_timer(int tid, unsigned long tick, int id, basics::numptr data)
{
	...
	case SC_PROVOKE:	// ƒvƒƒ{ƒbƒN/ƒI?ƒgƒo?ƒT?ƒN
		if(sc_data[type].integer2()!=0){	// ƒI?ƒgƒo?ƒT?ƒNi‚P•b‚²‚Æ‚ÉHPƒ`ƒFƒbƒNj
	...
	case SC_WEDDING:	//Œ‹¥—p(Œ‹¥ˆßÖ‚É‚È‚Á‚Ä?‚­‚Ì‚ª?‚¢‚Æ‚©)
		if(sd){
			time_t timer;
			if(time(&timer) < ((sc_data[type].integer2()) + 3600)){	//1ŠÔ‚½‚Á‚Ä‚¢‚È‚¢‚Ì‚Å??
				// ƒ^ƒCƒ}?Äİ’è
				sc_data[type].timer=add_timer(10000+tick, status_change_timer,bl->id, data);
				return 0;
			}
	...
	case SC_NOCHAT:	//ƒ`ƒƒƒbƒg‹Ö~?‘Ô
		if(sd && config.muting_players){
			time_t timer;
			if((++sd->status.manner) && time(&timer) < ((sc_data[type].integer2()) + 60*(0-sd->status.manner))){	//ŠJn‚©‚çstatus.manner•ª?‚Á‚Ä‚È‚¢‚Ì‚Å??
				clif_updatestatus(*sd,SP_MANNER);
				// ƒ^ƒCƒ}?Äİ’è(60•b)
