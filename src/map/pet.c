...
static int petskill_use(struct pet_data *pd, struct block_list *target, short skill_id, short skill_lv, unsigned int tick)
{
	...
	if(pd->msd->pet.intimate < 900)	// It doesn't like you [Aru]
		return 0;
...
{	
	...
	if (pd == NULL ||
		(battle_config.pet_equip_required && (pd->equip != sd->petDB->AcceID)) ||
		pd->recovery == NULL ||
		pd->recovery->timer != -1 ||
	...
	switch(pd->state.state){
		...
		case MS_ATTACK:
			...
			if (battle_config.pet_status_support &&
				pd->a_skill &&
				(!battle_config.pet_equip_required || pd->equip == pd->msd->petDB->AcceID) &&
...
int pet_equipitem(struct map_session_data *sd,int index)
{
	...
	if(sd->petDB->AcceID == 0 || nameid != sd->petDB->AcceID || sd->pet.equip != 0) {
		...
		if (battle_config.pet_equip_required || sd->petDB->AcceID == 0 )
