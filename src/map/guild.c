...
int guild_reqalliance(struct map_session_data *sd,int account_id)
{
	...
	if( guild_get_alliance_count(g[0],0)>=MAX_GUILDALLIANCE )	// 同盟数確認
		clif_guild_allianceack(sd,4);
	if( guild_get_alliance_count(g[1],0)>=MAX_GUILDALLIANCE )
...
{
#ifdef DISABLE_OPPOSITION
	clif_displaymessage("Sorry, oppositions not currently supported.");
#else
...
{
	static int agit_init_done = 0;
	int i;
	int n = (len-4) / sizeof(struct guild_castle), ev = -1;

	if(agit_init_done) return 0;
