...
bool fightable::is_movable()
{
	...
	if( sd )
	{
		if( sd->skilltimer != -1 && !sd->skill_check( SA_FREECAST) || sd->is_sitting() )
			return false;
	}
	else if( md )
	{
...
int unit_skilluse_id2(block_list *src, int target_id, int skill_num, int skill_lv, int casttime, int castcancel) {
	...
	if(sd) {
		...
		case BD_ENCORE:	
			...
			if(!sd->skillid_dance || sd.skill_check(sd->skillid_dance)<=0){
				sd.skill_failed(skill_num);
				return 0;
			}
	...
	if(casttime > 0) {
		...
		if(sd && sd.skill_check(SA_FREECAST))
...
static int unit_attack_timer_sub(block_list* src, int tid, unsigned int tick)
{
	...
	if(ud->skilltimer != -1 && (!sd || sd.skill_check(SA_FREECAST) <= 0))
		return 0;
	...
	if(!config.skill_delay_attack_enable &&
		...
		(!sd || sd.skill_check(SA_FREECAST) <= 0)
	) {
		if (tid == -1) { //requested attack.
			if(sd) sd.skill_failed(1,SF_DELAY);
			return 0;
		}
