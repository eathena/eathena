...
int status_percentrefinery(map_session_data &sd,struct item &item)
{
	...
	percent += sd.skill_check(BS_WEAPONRESEARCH);	// •ŠíŒ¤‹†ƒXƒLƒ‹Š
...
int status_calc_pc(map_session_data& sd, int first)
{
	...
	if (calculating < 10) //Too many recursive calls to status_calc_pc!
	{
		...
		if( (skill=sd.skill_check(AC_OWL))>0 )	// ‚Ó‚­‚ë‚¤‚Ì–Ú
			sd.paramb[4] += skill;
		...
		if((skill=sd.skill_check(AC_VULTURE))>0){	// ƒƒV‚Ì–Ú
			sd.hit += skill;
			if(sd.status.weapon == 11)
				sd.attackrange += skill;
		}

		if( (skill=sd.skill_check(BS_WEAPONRESEARCH))>0)	// •Ší?‹†‚Ì–½’†—¦?‰Á
			sd.hit += skill*2;
		if(sd.status.option&2 && (skill = sd.skill_check(RG_TUNNELDRIVE))>0 )	// ƒgƒ“ƒlƒ‹ƒhƒ‰ƒCƒu	// ƒgƒ“ƒlƒ‹ƒhƒ‰ƒCƒu
			sd.speed = sd.speed*100/(20+6*skill);
		if (sd.is_carton() && (skill=sd.skill_check(MC_PUSHCART))>0)	// ƒJ?ƒg‚É‚æ‚é‘¬“x’á‰º
			sd.speed += (10-skill) * DEFAULT_WALK_SPEED/10;
		if (sd.is_riding()) {	// ƒyƒRƒyƒR?‚è‚É‚æ‚é‘¬“x?‰Á
			sd.speed -= DEFAULT_WALK_SPEED/4;
			sd.max_weight += 10000;
		}
		if((skill=sd.skill_check(CR_TRUST))>0) { // ƒtƒFƒCƒX
		...
		if((skill=sd.skill_check(HP_MEDITATIO))>0) // ƒƒfƒBƒeƒCƒeƒBƒI
			sd.status.max_sp += sd.status.max_sp*skill/100;
		if((skill=sd.skill_check(HW_SOULDRAIN))>0) // ƒ\ƒEƒ‹ƒhƒŒƒCƒ“ 
		...
		if((skill=sd.skill_check(SM_RECOVERY)) > 0) {	// HP‰ñ•œ—ÍŒüã
		...
		if((skill=sd.skill_check(MG_SRECOVERY)) > 0) { // SP‰ñ•œ—ÍŒüã 
		...
		if( (skill=sd.skill_check(SA_DRAGONOLOGY))>0 ){ // ƒhƒ‰ƒSƒmƒƒW?
			skill = skill*4;
		...
		//Fleeã¸
		if( (skill=sd.skill_check(TF_MISS))>0 ){	// ‰ñ”ğ—¦?‰Á
		...
		if( (skill=sd.skill_check(MO_DODGE))>0 )	// Œ©Ø‚è
			sd.flee += (skill*3)>>1;
