...
int mmo_auth(struct mmo_account* account, int fd)
{
	...
	if( i != auth_num )
	{
		if (newaccount)
		{
			login_log("Attempt of creation of an already existant account (account: %s_%c, pass: %s, received pass: %s, ip: %s)" RETCODE, account->userid, account->userid[len-1], auth_dat[i].pass, account->passwd, ip);
		...
		{
			login_log("Invalid password (account: %s, pass: %s, received pass: %s, ip: %s)" RETCODE, account->userid, auth_dat[i].pass, (account->passwdenc) ? "[MD5]" : account->passwd, ip);
			return 1; // 1 = Incorrect Password
		}

		if( auth_dat[i].connect_until_time != 0 && auth_dat[i].connect_until_time < time(NULL) )
		{
			login_log("Connection refused (account: %s, pass: %s, expired ID, ip: %s)" RETCODE, account->userid, account->passwd, ip);
		...
		{	// account is banned
			...
			if( auth_dat[i].ban_until_time > time(NULL) ) { // always banned
				login_log("Connection refused (account: %s, pass: %s, banned until %s, ip: %s)" RETCODE, account->userid, account->passwd, tmpstr, ip);
				return 6; // 6 = Your are Prohibited to log in until %s
			} else { // ban is over
				login_log("End of ban (account: %s, pass: %s, previously banned until %s -> not more banned, ip: %s)" RETCODE, account->userid, account->passwd, tmpstr, ip);
		...
		if( auth_dat[i].state )
		{
			login_log("Connection refused (account: %s, pass: %s, state: %d, ip: %s)" RETCODE, account->userid, account->passwd, auth_dat[i].state, ip);
	...
	else
	{
		if( !newaccount )
		{
			login_log("Unknown account (account: %s, received pass: %s, ip: %s)" RETCODE, account->userid, account->passwd, ip);
		...
		else
		{
			...
			login_log("Account creation and authentification accepted (account %s (id: %d), pass: %s, sex: %c, connection with _F/_M, ip: %s)" RETCODE, account->userid, new_id, account->passwd, account->userid[len-1], ip);
