...
char databaseinit_filename[1024] = "conf/inter.conf";
...
///////////////////////////////////////////////////////////////////////////////
// login session data (only used for holding the md5key)
...
///////////////////////////////////////////////////////////////////////////////
// logging (txt)
//!! todo compound to class with sql looging
FILE *log_fp = NULL;
int log_login = 1;
...
int save_packet(int fd, const char* str, const char* ip_str)
{
	...
	{	size_t i,j;
		...
		struct timeval tv;
		time_t unixtime;
		FILE *logfp = safefopen(login_log_unknown_packets_filename, "a");
		if (logfp)
		{
			gettimeofday(&tv, NULL);
			unixtime = tv.tv_sec;
...
{	
	...
	else if (ld)
	{
		...
		} while(j<2 && !encpasswdok && (j++) != passwdenc);
		return encpasswdok;
	}
	return false;
}
...
// obsolete, use sending the account data with 2750 instead
		case 0x2715:
		{
			...
			if( email_check(email) )
			{
				CLoginAccount account;
				if( account_db.searchAccount(accid, account) &&
					(0==strcmp(account.email, "a@a.com") || account.email[0] == '\0') )
				{
					safestrcpy(account.email, email, sizeof(account.email));
					login_log("Char-server '%s': Create an e-mail on an account with a default e-mail (account: %d, new e-mail: %s, ip: %s)." RETCODE, server[id].name, accid, email, ip_str);
					// Save
					account_db.saveAccount(account);
				}
				else
				{
					login_log("Char-server '%s': Attempt to create an e-mail on an account with a default e-mail REFUSED - account doesn't exist or e-mail of account isn't default e-mail (account: %d, ip: %s)." RETCODE, server[id].name, accid, ip_str);
				}
			}
			else
			{
				login_log("Char-server '%s': Attempt to create an e-mail on an account with a default e-mail REFUSED - e-mail is invalid (account: %d, ip: %s)" RETCODE, server[id].name, accid, ip_str);
			}
			RFIFOSKIP(fd,46);
			break;
		}
	...
	while(RFIFOREST(fd) >= 2)
	{
		...
		case 0x01dd:	// Ask connection of a client (encryption mode)
		{
			...
			else
			{
				...
				int passwdenc = (command == 0x64) ? 0 : PASSWORDENC;
	...
	while (RFIFOREST(fd) >= 2) {
			...
			for(i = 0; i < AUTH_FIFO_SIZE; i++) 
			{
				    ...
				    !auth_fifo[i].delflag) 
				{
					...
					for(k = 0; k < auth_num; k++) {
						if (auth_dat[k].account_id == accid) {
							WFIFOW(fd,0) = 0x2729;	// Sending of the account_reg2
							WFIFOL(fd,4) = accid;
				...
				return 0;
		{
			...
			if( !email_check(email) )
				...
				for(i = 0; i < auth_num; i++) {
					if (auth_dat[i].account_id == accid && (strcmp(auth_dat[i].email, "a@a.com") == 0 || auth_dat[i].email[0] == '\0'))
					{
						...
						login_log("Char-server '%s': Create an e-mail on an account with a default e-mail (account: %d, new e-mail: %s, ip: %s)." RETCODE,
						          server[id].name, accid, email, ip_str);
						// Save
						mmo_auth_sync();
						break;
					}
					...
					if (isGM(accid) == 0) {
						...
						if (level_new_gm > 0) {
							...
							if ((fp = safefopen(GM_account_filename, "a")) != NULL) {
								...
								strftime(tmpstr, 23, date_format, localtime(&unixtime));
								fprintf(fp, RETCODE "// %s: @GM command on account %ld" RETCODE "%ld %d" RETCODE, tmpstr, accid, accid, level_new_gm);
								fclose(fp);
								WBUFL(buf,6) = level_new_gm;
								read_gm_account();
								send_GM_accounts();
		...
		case 0x2722:	// 0x2722 <account_id>.L <actual_e-mail>.40B <new_e-mail>.40B
				...
				return 0;
		  {
			...
			if( !email_check(actual_email) )
				...
				for(i = 0; i < auth_num; i++) {
					if (auth_dat[i].account_id == accid) {
						if (strcasecmp(auth_dat[i].email, actual_email) == 0) {
							memcpy(auth_dat[i].email, new_email, 40);
							login_log("Char-server '%s': Modify an e-mail on an account (@email GM command) (account: %d (%s), new e-mail: %s, ip: %s)." RETCODE,
							          server[id].name, accid, auth_dat[i].userid, new_email, ip_str);
							// Save
							mmo_auth_sync();
						} else
							login_log("Char-server '%s': Attempt to modify an e-mail on an account (@email GM command), but actual e-mail is incorrect (account: %d (%s), actual e-mail: %s, proposed e-mail: %s, ip: %s)." RETCODE,
							          server[id].name, accid, auth_dat[i].userid, auth_dat[i].email, actual_email, ip_str);
						break;
					}
				}
				if (i == auth_num)
				...
				for(i = 0; i < auth_num; i++) {
					if (auth_dat[i].account_id == accid) {
						if (auth_dat[i].state != statut) {
							login_log("Char-server '%s': Status change (account: %d, new status %d, ip: %s)." RETCODE,
							          server[id].name, accid, statut, ip_str);
							if (statut != 0) {
								unsigned char buf[16];
								WBUFW(buf,0) = 0x2731;
								WBUFL(buf,2) = accid;
								...
								for(j = 0; j < AUTH_FIFO_SIZE; j++)
									if (auth_fifo[j].account_id == accid)
...
//					ShowMessage("%d,", auth_dat[i].account_id);
					if (auth_dat[i].account_id == accid) {
						if (auth_dat[i].sex == 2)
							login_log("Char-server '%s': Error of sex change - Server account (suggested account: %d, actual sex %d (Server), ip: %s)." RETCODE,
							          server[id].name, accid, auth_dat[i].sex, ip_str);
						else {
							unsigned char buf[16];
							if (auth_dat[i].sex == 0)
								sex = 1;
		...
		case 0x793e:	// Request to modify GM level
			...
			memcpy(WFIFOP(fd,6), account_name, 24);
			{
				...
				if (new_gm_level < 0 || new_gm_level > 99) {
					...
					i = search_account_index(account_name);
					if (i != -1) {
						int accid = auth_dat[i].account_id;
						memcpy(WFIFOP(fd,6), auth_dat[i].userid, 24);
						if (isGM(accid) != new_gm_level) {
							...
							if ((fp2 = lock_fopen(GM_account_filename, &lock)) != NULL) {
								if ((fp = safefopen(GM_account_filename, "r")) != NULL) {
									...
									while(fgets(line, sizeof(line), fp)) {
										...
										if( !skip_empty_line(line) )
											...
											if (sscanf(line, "%d %d", &GM_account, &GM_level) != 2 && sscanf(line, "%d: %d", &GM_account, &GM_level) != 2)
												fprintf(fp2, "%s" RETCODE, line);
											else if (GM_account != accid)
												fprintf(fp2, "%s" RETCODE, line);
											else if (new_gm_level < 1) {
												fprintf(fp2, "// %s: 'ladmin' GM level removed on account %d '%s' (previous level: %d)" RETCODE "//%d %d" RETCODE, tmpstr, accid, auth_dat[i].userid, GM_level, accid, new_gm_level);
												modify_flag = 1;
											} else {
												fprintf(fp2, "// %s: 'ladmin' GM level on account %d '%s' (previous level: %d)" RETCODE "%d %d" RETCODE, tmpstr, accid, auth_dat[i].userid, GM_level, accid, new_gm_level);
												modify_flag = 1;
											}
										}
									}
									if (modify_flag == 0)
										fprintf(fp2, "// %s: 'ladmin' GM level on account %d '%s' (previous level: 0)" RETCODE "%d %d" RETCODE, tmpstr, accid, auth_dat[i].userid, accid, new_gm_level);
									fclose(fp);
								} else {
									login_log("'ladmin': Attempt to modify of a GM level - impossible to read GM accounts file (account: %s (%d), received GM level: %d, ip: %s)" RETCODE,
									          auth_dat[i].userid, accid, (int)new_gm_level, ip_str);
								}
