...
// Borrowed from NetHack code by Aru
char * eos(s)
    char *s;
{
    while (*s) s++;
...
int delete_char_sql(int char_id, int partner_id)
{
	...
	if (sql_res && sql_row[0]) {
		...
		if(mysql_query(&mysql_handle, tmp_sql)) {
			printf("%s - DB server Error (hatched pet): %s\n", function_name, mysql_error(&mysql_handle));
		}
...
void mmo_charmenu_fromsql(int char_id, struct mmo_charstatus *p){
	...
	if (mysql_query(&mysql_handle, tmp_sql)) {
		printf("DB server Error (charmenu_fromsql)- %s\n", mysql_error(&mysql_handle));
	}
	...
	if ((sql_res = mysql_store_result(&mysql_handle))) {
        ...
        if (0){	//!strcmp(sql_row[36],"")
			strcpy(p->last_point.map,start_point.map);
			p->last_point.x = start_point.x;
			p->last_point.y = start_point.y;
		} else {
			strcpy(p->last_point.map,sql_row[36]);
	        	p->last_point.x = atoi(sql_row[37]);
        		p->last_point.y = atoi(sql_row[38]);
		}
		if (0) { // !strcmp(sql_row[39],"")
			strcpy(p->save_point.map,start_point.map);
	        	p->save_point.x = start_point.x;
        		p->save_point.y = start_point.y;
		} else{
...
void read_gm_account(void) {
	...
	sprintf(tmp_lsql, "SELECT `%s`,`%s` FROM `%s` WHERE `%s`>='%d'",login_db_account_id,login_db_level,login_db,login_db_level,1);
...
//}//---------------------------test count------------------------------
	...
	sprintf(tmp_sql ,"UPDATE `%s` SET `class`='%d', `base_level`='%d', `job_level`='%d',"
		"`base_exp`='%d', `job_exp`='%d', `zeny`='%d', `guild_id`='%d', "
		"`max_hp`='%d',`hp`='%d',`max_sp`='%d',`sp`='%d',`status_point`='%d',`skill_point`='%d',"
		"`str`='%d',`agi`='%d',`vit`='%d',`int`='%d',`dex`='%d',`luk`='%d',"
		"`option`='%d',`karma`='%d',`manner`='%d',`party_id`='%d',`pet_id`='%d',"
		...
		p->base_exp, p->job_exp, p->zeny, p->guild_id,
		p->max_hp, p->hp, p->max_sp, p->sp, p->status_point, p->skill_point,
		p->str, p->agi, p->vit, p->int_, p->dex, p->luk,
		p->option, p->karma, p->manner, p->party_id, p->pet_id,
	...
	for(i=0;i<3;i++){
		if(p->memo_point[i].map[0]){
			sprintf(tmp_sql,"REPLACE INTO `%s`(`char_id`,`memo_id`,`map`,`x`,`y`)"
				" VALUES ('%d', '%d', '%s', '%d', '%d')",
				memo_db, char_id, i, p->memo_point[i].map, p->memo_point[i].x, p->memo_point[i].y);
			if(mysql_query(&mysql_handle, tmp_sql))
...
//	printf("(\033[1;13m%d\033[0m) Request Char Data:\n",sd->account_id);
	...
	for(i = 0; i < found_num; i++) {
		mmo_charmenu_fromsql(sd->found_char[i], char_dat);
