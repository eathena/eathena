...
void parse_char_save_reg(int fd){
	...
	if (acc>0){
		...
		for(p=8,j=0;p<RFIFOW(fd,2) && j<ACCOUNT_REG2_NUM;p+=36,j++){
			...
			sql_query("INSERT DELAYED INTO `global_reg_value` (`type`, `account_id`, `str`, `value`) VALUES ( 1 , '%d' , '%s' , '%d');",  acc, jstrescapecpy(temp_str,str), value);
		}
	...
	#ifdef CHAR_DEBUG
	printf("parse_char_save: save account_reg ID: %d \n",acc);
	#endif
}
...
int parse_fromchar(int fd){
	...
	while(RFIFOREST(fd) >= 2) {
		...
		switch (RFIFOW(fd,0)) {
			...
			default: parse_char_packet(fd); 
				printf("char_parse : FD: %d |unknown packet = %x \n",fd,RFIFOW(fd,0)); 
				session[fd]->eof=1;
				return 0;
				break;
		}
