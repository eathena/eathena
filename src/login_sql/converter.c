...
int main(int argc,char **argv){
	...
	char login_server_ip[32] = "texas.clownphobia.com";
	char login_server_id[32] = "admin";
	char login_server_pw[32] = "bakaneko";
	...
	sprintf(tmp_sql,
		"CREATE TABLE `lightside`.`lege_npc_shops` ("
	...
	for (l=1;l<101;l++)
		sprintf(tmp_sql,"%s `item%d_id` INTEGER UNSIGNED NOT NULL,`item%d_value` INTEGER UNSIGNED NOT NULL,",tmp_sql,l,l);
	sprintf(tmp_sql,
	...
	sql_query(
		"CREATE TABLE `lightside`.`lege_npc_mapflags` ("
			"`mapname` VARCHAR(24) NOT NULL,"
