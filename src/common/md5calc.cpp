...
// current changes are just temporary; this code will be replaced anyway
...
void MD5_String2binary(const char * str, char * output)
{
	...
	unsigned int string_bit_len;	//The bit length of string is held.
	unsigned int copy_len;			//The number of bytes which is used by 1-3 and which remained
	unsigned int msg_digest[4];		//Message digest   128bit 4byte
	...
	*D = 0x10325476;
	...
	// have it endian safe
	padding_message[56+0] = (unsigned char)((string_bit_len      ) & 0xff);
	padding_message[56+1] = (unsigned char)((string_bit_len >>  8) & 0xff);
	padding_message[56+2] = (unsigned char)((string_bit_len >> 16) & 0xff);
	padding_message[56+3] = (unsigned char)((string_bit_len >> 24) & 0xff);
	...
	if(UINT_MAX / 8 < string_byte_len)
	{
		...
		// have it endian safe
		padding_message[60+0] = (unsigned char)((high      ) & 0xff);
		padding_message[60+1] = (unsigned char)((high >>  8) & 0xff);
		padding_message[60+2] = (unsigned char)((high >> 16) & 0xff);
		padding_message[60+3] = (unsigned char)((high >> 24) & 0xff);
	}
	else
	...
	// have it endian safe
	output[ 0] = (unsigned char)((msg_digest[0]      ) & 0xff);
	output[ 1] = (unsigned char)((msg_digest[0] >>  8) & 0xff);
	output[ 2] = (unsigned char)((msg_digest[0] >> 16) & 0xff);
	output[ 3] = (unsigned char)((msg_digest[0] >> 24) & 0xff);
	output[ 4] = (unsigned char)((msg_digest[1]      ) & 0xff);
	output[ 5] = (unsigned char)((msg_digest[1] >>  8) & 0xff);
	output[ 6] = (unsigned char)((msg_digest[1] >> 16) & 0xff);
	output[ 7] = (unsigned char)((msg_digest[1] >> 24) & 0xff);
	output[ 8] = (unsigned char)((msg_digest[2]      ) & 0xff);
	output[ 9] = (unsigned char)((msg_digest[2] >>  8) & 0xff);
	output[10] = (unsigned char)((msg_digest[2] >> 16) & 0xff);
	output[11] = (unsigned char)((msg_digest[2] >> 24) & 0xff);
	output[12] = (unsigned char)((msg_digest[3]      ) & 0xff);
	output[13] = (unsigned char)((msg_digest[3] >>  8) & 0xff);
	output[14] = (unsigned char)((msg_digest[3] >> 16) & 0xff);
	output[15] = (unsigned char)((msg_digest[3] >> 24) & 0xff);
}
