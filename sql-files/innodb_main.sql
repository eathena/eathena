...
CREATE TABLE `login` (
  ...
  KEY `name` (`userid`)
) TYPE=InnoDB AUTO_INCREMENT=1000000; 
...
# Database: Ragnarok
# Table: 'account_reg_value'
# 
DROP TABLE IF EXISTS `account_reg_value`;
...
CREATE TABLE `party` (
  ...
  FOREIGN KEY (`leader_id`) REFERENCES `char` (`char_id`) ON DELETE CASCADE
) TYPE=InnoDB; 
